...
```javascript
BugsterTracker.setEnvironment('staging', {
  baseUrl: 'https://staging.example.com',
  apiKey: 'staging-api-key'
});

BugsterTracker.runTest('login-flow', { environment: 'staging' });
```
...
```javascript
BugsterTracker.createDataSet('login-credentials', [
  { username: 'user1@example.com', password: 'pass1' },
  { username: 'user2@example.com', password: 'pass2' }
]);

BugsterTracker.runTest('login-flow', { dataSet: 'login-credentials' });
```
...
```javascript
BugsterTracker.setTestDependency('checkout-flow', ['login-flow', 'add-to-cart']);
```
...
```javascript
BugsterTracker.enableFlakyDetection('login-flow', {
